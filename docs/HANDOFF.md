...
> 최소청약주식수 및 LLM 기반 청약 AI 진단 리포트 연동 내역이 포함되어 있습니다.

# Handoff notes (2026-09-17) — 공모주(IPO) 데이터 고도화 및 LLM 청약 AI 진단 리포트 연동
...
### 1) 38커뮤니케이션(`38.co.kr`) 크롤링 파이프라인 구축 (`src/lib/ipo-scraper.ts`)
...
- **EUC-KR 디코딩 및 실시간 파싱**:
  ...
  - `o=v&no=...` (종목별 상세 페이지): `인수회사` 테이블과 `주간사` 행을 파싱하여 증권사별 실제 배정수량, 1인 최고 청약한도, 희망공모가 밴드 수집.
- **성능 최적화 및 캐싱**:
...
### 2) 복수 주관사 개별 분리 노출 및 증권사명 표준화 (`src/lib/dart.ts`, `src/lib/ipo-scraper.ts`)
...
### 3) 최소 청약 주식수(기본 10주) 및 필요 증거금(50%) 추가 (`src/lib/types.ts`, `src/lib/dart.ts`)
- **타입 및 계산 로직 추가**:
...
### 4) LLM 기반 데이터 중심 청약 AI 진단 리포트 (`src/lib/ipo-analyzer.ts`, `src/app/api/ipos/analyze/route.ts`)
...
- **프론트엔드 연동 (`src/app/ipo/page.tsx`)**:
  ...
  - 상세 뷰: `IpoAiReportCard` 컴포넌트를 통해 진단 요약, 호재/주의점 2열 그리드, 추천 청약 전략 박스 렌더링.
...
- `npm run build`: Next.js 16.3.2 Turbopack 전체 26개 라우트(신규 `/api/ipos/analyze` 포함) 빌드 통과 (`exit code 0`).
- 실제 엔드포인트 테스트 완료:
  - `/api/ipos` 및 `/api/ipos/analyze` 실데이터 호출 검증 완료.
  - 브릴스(적극 추천, 95점), 엘리스그룹(2개 주관사 분리), 디티에스(2개 주관사 분리), 글로벌테크놀로지(패스 권고, 15점) 정상 반환 확인.
- Git 동기화:
  - 리포지토리 리네임(`nesio` → `nescio`) URL 갱신 완료 (`https://github.com/heartyhong2002-png/nescio.git`).
  - 모든 변경사항 커밋 및 `origin/main` 푸시 완료 (`9fae974`).
