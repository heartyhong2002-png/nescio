...
# Handoff notes (2026-09-21) — [개발 세션 작업 지시서] 증권사 리포트 & 전문가 의견 수집 및 AI 요약 파이프라인
...
- **문제점**:
  - 지금까지는 네이버 뉴스만 수집하여 주가 변동 원인을 분석했으나, 뉴스는 '사후 약방문(이미 주가가 오른 뒤에 붙인 기사)'이거나 단기 이슈/자극적 헤드라인에 치우치는 한계가 있음.
  - 반면 **증권사 리포트(애널리스트 보고서)**는 기업의 실적 전망치(컨센서스), 목표주가, 밸류에이션 등 가장 신뢰할 수 있는 데이터를 담고 있으나, **"용어가 너무 어렵고 수십 페이지에 달해 초보자는 읽을 수 없다"**는 높은 진입장벽이 존재함.
- **해결책**:
  1. 종목별 **증권사 컨센서스(목표주가, 투자의견 점수) 및 최신 리포트 메타데이터**를 수집.
  2. **오픈소스 LLM / Gemini**를 통해 어려운 금융 용어를 초등학생도 이해할 수 있는 **"3줄 전문가 뷰 요약"**으로 재해석.
  3. 종목 상세 페이지에 **[단기 뉴스 이슈 vs 중장기 전문가 전망]**을 함께 보여주는 입체적 인텔리전스 제공.

---

## 2. 세부 기능 및 데이터 소스 명세 (Functional Requirements)

### ① 데이터 소스 전략 (2-Track 연동)
1. **트랙 A (정량 수치) - KIS (한국투자증권) API**:
   - 종목별 증권사 평균 목표주가, 투자의견 점수(5점 만점 기준), 추정 EPS/PER 컨센서스 조회.
   - 공식 API이므로 차단 위험 없이 빠르고 정확한 수치 산출.
2. **트랙 B (정성 리포트) - 네이버 증권 종목 리서치 크롤러 (`src/lib/consensus-scraper.ts`)**:
   - URL: `https://finance.naver.com/research/company_list.naver?keyword=&brokerCode=&searchType=itemCode&itemCode={ticker}`
   - 수집 대상 (최근 3~5건):
     - `broker`: 발행 증권사 (예: 미래에셋증권, 삼성증권)
     - `date`: 작성일자 (YYYY-MM-DD)
     - `title`: 리포트 제목 (예: "HBM3E 수율 안정화로 하반기 어닝 서프라이즈 기대")
     - `targetPrice`: 해당 리포트의 목표주가 (원 단위 정수)
     - `opinion`: 투자의견 (매수/BUY, 중립/HOLD 등)
     - `attachmentUrl`: 원문 PDF 다운로드 링크 (네이버 증권 링크)

### ② AI 전문가 리포트 초보자 요약 엔진 (`src/lib/consensus-analyzer.ts`)
- **역할**: 수집된 3~5개 증권사 리포트의 제목, 목표가, 기존 시세/뉴스를 바탕으로 친절한 3줄 요약 생성.
- **프롬프트 페르소나**:
  - *"여의도 전문가들의 뷰를 주식 초보자 친구에게 설명해 주는 친절한 분석가"*
  - 전문 용어(12M Fwd P/E, CAPEX, 컨센서스 등)는 쉽게 풀어서 설명.
- **출력 스키마 (`ConsensusAiReport`)**:
  - `consensusScore`: 5점 만점 환산 점수
  - `averageTargetPrice`: 평균 목표주가 (원)
  - `upsidePercent`: 현재가 대비 상승 여력 (`(목표가 - 현재가) / 현재가 * 100`)
  - `summary`: 3문장의 친절한 전문가 뷰 요약
  - `keyDrivers`: 전문가들이 주목하는 2~3가지 핵심 성장 요인/리스크 요인
- **다중 LLM 폴백**: 기존 프로젝트 표준인 Groq(Llama-3.3/3.1) $\rightarrow$ Gemini $\rightarrow$ 무중단 룰베이스 폴백 유지.

### ③ 소형주(스몰캡) 리포트 미발행 종목 예외 처리 (Fallback)
- 코스닥 중소형주는 증권사 리포트가 전혀 없는 경우가 빈번함.
- **대응 UX**: 에러를 띄우지 않고, *"아직 증권사 공식 리포트가 발간되지 않은 종목이에요. 상단의 실시간 뉴스 분석을 참고해 보세요!"* 카드로 매끄럽게 Fallback 처리.

---

## 3. UI/UX 디자인 가이드 (`src/app/stock/[ticker]/page.tsx` 확장)

종목 상세 페이지의 뉴스 분석 카드 아래에 **[🏛️ 증권사 전문가 리포트 & 컨센서스]** 섹션을 추가:

```
┌────────────────────────────────────────────────────────┐
│ 🏛️ 증권사 전문가 컨센서스 (목표가 & 리포트)             │
├────────────────────────────────────────────────────────┤
│  현재가 72,000원   ➔   평균 목표가 98,000원 (상승여력 +36.1%)  │
│  투자의견: [🔥 매수 (BUY)] (평균 평점 4.1 / 5.0)          │
├────────────────────────────────────────────────────────┤
│ 🤖 AI 전문가 뷰 3줄 요약:                               │
│ • "여의도 증권사들은 지금 주가를 강한 저평가 상태로 봐요!"  │
│ • "최근 5개 증권사 모두 목표가를 9.5~10.5만 원으로 유지했어요."│
│ • "핵심 무기로는 '차세대 HBM 신제품 납품'을 공통으로 꼽았어요."│
├────────────────────────────────────────────────────────┤
│ 📋 최근 발간된 리포트 (최근 3건):                        │
│ • [미래에셋] HBM3E 독주로 하반기 최대 실적 (목표가 10.5만)  │
│ • [삼성증권] 단기 노이즈보다 실적 개선에 집중 (목표가 9.8만)  │
│ • [한국투자] 사이클 저점 통과, 비중 확대 권고 (목표가 9.5만) │
└────────────────────────────────────────────────────────┘
```

---

## 4. 권장 구현 아키텍처 및 신규 파일 목록

새 개발 세션에서는 다음 구조로 모듈화를 진행하는 것을 권장합니다:

```
src/
  lib/
    consensus-types.ts       # AnalystReport, StockConsensus, ConsensusAiReport 타입 정의
    consensus-scraper.ts     # 네이버 증권 리서치 HTML 파서 (30분 인메모리 캐시)
    consensus-analyzer.ts    # Groq / Gemini / 룰베이스 요약 생성 엔진
  app/
    api/
      stock/
        [ticker]/
          consensus/
            route.ts         # 종목별 컨센서스 + 최신 리포트 + AI 요약 반환 API (GET)
  components/
    StockConsensusCard.tsx   # 목표가 게이지, AI 3줄 요약, 리포트 타임라인 렌더링 컴포넌트
```
...
1. `npm run build` 실행 시 Turbopack 빌드가 에러(`exit code 0`) 없이 통과할 것.
2. 대형주(예: 삼성전자 `005930`, SK하이닉스 `000660`) 조회 시 최신 리포트와 목표가가 정상 노출될 것.
3. 리포트가 없는 스몰캡 종목에서도 페이지가 깨지지 않고 친절한 안내 카드로 Fallback될 것.
4. 외부 API/크롤링 지연 시에도 30분 캐시를 적용하여 1초 이내에 빠르게 렌더링될 것.

---

> **이전 세션 인수인계 메모 (9/21 이전):**
