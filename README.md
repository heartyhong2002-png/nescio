...
6. 글로벌 매크로(유가·채권·원자재) & 오픈소스 LLM 증시 영향 코멘터리 & 국제 회의 캘린더 (개발 예정):
   - **Top-Pick 매크로 핵심 지표**: 유가(WTI), 금, 구리, 미 국채 10Y/2Y, 달러인덱스, 원/달러, VIX 등 7대 핵심 지표 선별 시각화.
   - **오픈소스 LLM(Llama-3) 시장 영향 브리핑**: 현재 거시 지표 변동이 왜 주식 시장에 영향을 미치는지 직관적인 한 줄 해설 및 종합 브리핑 제공.
   - **주요 국제 회의 캘린더 (Upcoming Events)**: FOMC, OPEC+, BOJ 등 글로벌 주요 정책 회의 일정(D-Day) 및 증시 관전 포인트 제공.
